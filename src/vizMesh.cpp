...
void vizMesh::setup(){
    ...
    ofLogNotice() << mVizIdName << "::setup()" << endl;
    // - - - - - - - - - - OFX UI SETUP - - - - - - - - -  - - - - -
    ...
    //  - - - - - - - - - - MESH - - - - - - - - - - - - - - - - -
    gui->addWidgetDown(new ofxUILabel("viz mesh", OFX_UI_FONT_LARGE));
    ...
    ofAddListener(gui->newGUIEvent,this,&vizMesh::guiEvent);
...
//--------------------------------------------------------------
void vizMesh::update( list<ofxTuioCursor*> cursors) {
...
//--------------------------------------------------------------
void vizMesh::update(ofVec3f target) {
    //gives better feeback when selecting presets, does not change the actual value though
    mSelectedPreset = int(mSelectedPreset);
...
//--------------------------------------------------------------
void vizMesh::keyPressed(int key) {
...
//--------------------------------------------------------------
void vizMesh::guiEvent(ofxUIEventArgs &e){
